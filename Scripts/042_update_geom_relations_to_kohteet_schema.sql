...
BEGIN
    ...
    IF table_name IN ('keskilinja', 'viheralueenosa', 'katualueenosa', 'ajoratamerkinta', 'hulevesi', 'jate', 'kaluste', 'leikkivaline', 'liikennemerkki', 'liikunta', 'melu', 'muuvaruste', 'opaste', 'pysakointiruutu', 'rakenne', 'ymparistotaide') THEN
...
drop trigger if exists geom_relations ON kohteet.keskilinja;
create trigger geom_relations after
insert or update of geom
on kohteet.keskilinja for each statement execute function kohteet.geom_relations();
...
on kohteet.katualueenosa for each statement execute function kohteet.geom_relations();

drop trigger if exists geom_relations ON kohteet.viheralueenosa;
create trigger geom_relations after
insert or update of geom, loppuhetki
on kohteet.viheralueenosa for each statement execute function kohteet.geom_relations();
