...
---
--- NOTE: This trigger should not currently be used for normal updates, use the "_for_each_row" version instead.
--- The reason is, that this trigger function does too much work, because it does not know which rows have changed.
--- Let's keep it around anyway for reference.
---
--- For tracking changes with variables, here's a trick that might be relevant (or maybe not, but interesting anyway)
---
...
-- ),
-- template AS (
--     SELECT 'drop trigger if exists geom_relations ON kohteet.__TABLE__;
-- create trigger geom_relations after
-- insert or update of geom_piste, geom_line, geom_poly, loppuhetki
-- on kohteet.__TABLE__ for each statement execute function kohteet.geom_relations();' AS cmd
-- )
-- SELECT replace(template.cmd, '__TABLE__', tables.tablename) FROM tables CROSS JOIN template;
