---
--- geom_relations_for_each_row trigger function
---
--- Note that this trigger function does too much work, because it does not know which rows have changed.
--- Might need to revisit, if this causes table bloat or too long runtime.
---
--- Here's a trick that might be relevant (or maybe not, but interesting anyway)
---
...
-- ),
-- template AS (
--     SELECT 'drop trigger if exists geom_relations_for_each_row ON kohteet.__TABLE__;
-- create trigger geom_relations_for_each_row after
-- insert or update of geom_piste, geom_line, geom_poly, loppuhetki
-- on kohteet.__TABLE__ for each row execute function kohteet.geom_relations_for_each_row();' AS cmd
-- )
