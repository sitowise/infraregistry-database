...
try
{
    ...
    string municipalityCode = args[3];
    ...
    UpgradeEngine upgrader = CreateUpgradeEngine(connectionString, srid, municipalityCode, s => !s.StartsWith("InfraRegistry.DbUp.Scripts.views"));
    UpgradeEngine viewUpgrader = CreateUpgradeEngine(connectionString, srid, municipalityCode, s => s.StartsWith("InfraRegistry.DbUp.Scripts.views"), useNullJournal: true);
    ...
    {
        List<SqlScript> scripts = upgrader.GetScriptsToExecute();

        Console.WriteLine("Scripts that need to be run:");
        foreach (SqlScript? sc in scripts)
        {
...
static UpgradeEngine CreateUpgradeEngine(string connectionString, int srid, string municipalityCode, Func<string, bool> filterScripts, bool useNullJournal = false)
{
    ...
    string? databaseName = connStringBuilder.Database;
    string? systemUsername = connStringBuilder.Username;

    UpgradeEngineBuilder upgraderBuilder = DeployChanges.To
